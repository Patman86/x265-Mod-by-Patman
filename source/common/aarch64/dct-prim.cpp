...
// Stride-aware version: reads from stride layout, skips intermediate buffer
template<int shift>
static inline void partialButterfly16_neon_stride(const int16_t *src, int16_t *dst, intptr_t srcStride)
{
    const int line = 16;
    ...
    // Load directly from stride layout
    for (int i = 0; i < line; i += 2)
    {
        // Load row i and i+1 from stride layout (each row has 16 elements = 2 vectors)
...
// Stride-aware version: reads from stride layout, skips intermediate buffer
template<int shift>
static inline void partialButterfly32_neon_stride(const int16_t *src, int16_t *dst, intptr_t srcStride)
{
    const int line = 32;
    ...
    for (int i = 0; i < line; i += 2)
    {
        // Load row i and i+1 from stride layout (each row has 32 elements = 4 vectors)
    ...
    for (int k = 1; k < 32; k += 2)
    {
        ...
        for (int i = 0; i < line; i += 4)
        {
            ...
            int32x4_t t01 = vpaddq_s32(t[0], t[1]);
            int32x4_t t23 = vpaddq_s32(t[2], t[3]);
    ...
    for (int k = 2; k < 32; k += 4)
    {
        ...
        for (int i = 0; i < line; i += 4)
        {
            int32x4_t t0 = vmulq_s32(c0, EO[i + 0][0]);
            int32x4_t t1 = vmulq_s32(c0, EO[i + 1][0]);
            int32x4_t t2 = vmulq_s32(c0, EO[i + 2][0]);
            int32x4_t t3 = vmulq_s32(c0, EO[i + 3][0]);
            t0 = vmlaq_s32(t0, c1, EO[i + 0][1]);
            t1 = vmlaq_s32(t1, c1, EO[i + 1][1]);
            t2 = vmlaq_s32(t2, c1, EO[i + 2][1]);
            t3 = vmlaq_s32(t3, c1, EO[i + 3][1]);
...
void dct16_neon(const int16_t *src, int16_t *dst, intptr_t srcStride)
{
    ...
    // Use stride-aware version to skip intermediate buffer copy
    partialButterfly16_neon_stride<shift_pass1>(src, coef, srcStride);
...
void dct32_neon(const int16_t *src, int16_t *dst, intptr_t srcStride)
{
    ...
    // Use stride-aware version to skip intermediate buffer copy
    partialButterfly32_neon_stride<shift_pass1>(src, coef, srcStride);
